  ...
  return (
    <>
      {!isProductDetail && (
        ...
        >
      <div className="mx-auto flex h-12 max-w-[1280px] items-center justify-between px-4 md:h-14 md:px-8">
        {/* Wordmark */}
        <Link
          href="/"
        ...
        >
          <Image
            ...
            height={36}
            priority
            className="h-8 w-8 rounded-full object-cover ring-1 ring-line md:h-9 md:w-9"
          />
          <span className="font-sans text-[13px] font-bold leading-none tracking-[-0.02em] md:text-sm">
            {STORE.name}
          </span>
        </Link>
      ...
      >
          {NAV_LINKS.map((link) => {
            ...
            return (
              <Link
                ...
                href={link.href}
                data-active={active}
                className="flex h-12 flex-col items-center justify-center gap-0.5 border-r border-line text-[8px] font-medium uppercase tracking-[0.12em] text-ink-soft transition-colors last:border-r-0 data-[active=true]:text-ink data-[active=true]:[&>span]:[&>svg]:stroke-[2.2]"
              >
          ...
          <Link
            ...
            className="relative flex h-12 flex-col items-center justify-center gap-0.5 text-[8px] font-medium uppercase tracking-[0.12em] text-ink-soft transition-colors data-[active=true]:text-ink data-[active=true]:[&>span>svg]:stroke-[2.2]"
          >
            <span aria-hidden className="flex h-4 w-4 items-center justify-center">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="h-4 w-4">
            ...
            Cart
            {cartCount > 0 && (
              <span className="absolute top-0.5 right-[calc(50%-16px)] flex h-3.5 min-w-3.5 items-center justify-center rounded-full bg-terracotta px-0.5 text-[7px] font-bold text-white">
